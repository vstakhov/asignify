...
bcrypt_hash(uint8_t *sha2pass, uint8_t *sha2salt, uint8_t *out)
{
	...
	/* zap */
	explicit_memzero(ciphertext, sizeof(ciphertext));
	explicit_memzero(cdata, sizeof(cdata));
    ...
    uint8_t *key, size_t keylen, unsigned int rounds)
{
	...
	/* collapse password */
	SHA512Init(&ctx);
	SHA512Update(&ctx, (const u_int8_t *)pass, passlen);
	...
	/* zap */
	explicit_memzero(&ctx, sizeof(ctx));
	explicit_memzero(out, sizeof(out));
